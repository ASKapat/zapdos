#include "SputterE_BC.h"

registerADMooseObject("ZapdosApp", SputterE_BC);

InputParameters
SputterE_BC::validParams()
{
InputParameters params = ADIntegratedBC::validParams();
  ...
  params.addRequiredParam<Real>("E_SB", "The surface binding energy of target.");
  //Energy of the sputtered species for now approximated by 1/2 surface binding energy
...
SputterE_BC::SputterE_BC(const InputParameters & parameters)
	: ADIntegratedBC(parameters),
	  _r_units(1. /getParam<Real>("position_units")),
	  _kb(getMaterialProperty<Real>("k_boltz")),
	  _Zs(getParam<Real>("sputter_Z")),
	  _E_sb(getParam<Real>("E_SB")),
...
//     std::cout << i << std::endl;
  ...
  _ion_flux = ((1-_r_ion[i])/(1+_r_ion[i]))*((std::exp((*_n_ip[i])[_qp])) * (0.5 * _v_th_i +(2 * _a - 1) *
...
//   std::cout << _r_units * (1. - _a * _sgn_s[_qp]) * _sputt_flux << std::endl;
  ...
  return -_test[_i][_qp] * _r_units * (1. - _a * _sgn_s[_qp])* _E_sb * _sputt_flux;
